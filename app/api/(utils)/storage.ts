...
interface AppStorage {
    ...
    getReleaseFile(type: "scripts" | "metadata", moduleName: string, releaseId: string): Promise<Buffer>;

    // TODO: Don't share sharp.Sharp
    setImage(type: "module" | "user", name: string, file: sharp.Sharp): Promise<string>;
    setReleaseFile(type: "scripts" | "metadata", moduleName: string, releaseId: string, file: FileWritable): Promise<void>;
...
class FileStorage implements AppStorage {
    ...
    async getReleaseFile(type: "scripts" | "metadata", moduleName: string, releaseId: string): Promise<Buffer> {
    ...
    async setReleaseFile(type: "scripts" | "metadata", moduleName: string, releaseId: string, file: FileWritable): Promise<void> {
...
export function createStorageFromEnv(s3EnvVar: string | undefined, localStorageEnvVar: string | undefined): AppStorage {
    const s3 = s3EnvVar && process.env[s3EnvVar];
    const localStorage = localStorageEnvVar && process.env[localStorageEnvVar];

    if (s3 && localStorage) {
        throw new Error(`Cannot specify both ${s3EnvVar} and ${localStorageEnvVar} environment variables`);
    }
